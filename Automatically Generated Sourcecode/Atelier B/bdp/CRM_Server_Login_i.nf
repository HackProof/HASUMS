...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(CRM_Server_Login_i),access_grant)==(btrue | Input_ID = Vehicle_ID & Input_Agent = Vehicle & Login_Try<=5 ==> (Agent:=Access_Success;Permission:=Download;(0: INT | Login_Try:=0);("Login Success!\n": STRING | skip);("Login Entity: Vehicle\n": STRING | skip);("============================\n": STRING | skip)) [] not(Input_ID = Vehicle_ID & Input_Agent = Vehicle & Login_Try<=5) ==> (Agent:=Access_Fail;Permission:=None;(Login_Try+1: INT & Login_Try: INT & 1: INT | Login_Try:=Login_Try+1);("Login Fail!\n": STRING | skip);("============================\n": STRING | skip)));
  List_Substitution(Implementation(CRM_Server_Login_i),access_grant)==(IF Input_ID = Vehicle_ID & Input_Agent = Vehicle & Login_Try<=5 THEN Agent:=Access_Success;Permission:=Download;Login_Try:=0;STRING_WRITE("Login Success!\n");STRING_WRITE("Login Entity: Vehicle\n");STRING_WRITE("============================\n") ELSE Agent:=Access_Fail;Permission:=None;Login_Try:=Login_Try+1;STRING_WRITE("Login Fail!\n");STRING_WRITE("============================\n") END)
END
&
...
THEORY ListSeenInfoX IS
  Seen_Internal_List_Operations(Implementation(CRM_Server_Login_i),Machine(BASIC_IO))==(INTERVAL_READ,INT_WRITE,STRING_WRITE);
  ...
  Seen_List_Precondition(Implementation(CRM_Server_Login_i),STRING_WRITE)==(ss: STRING);
  Seen_Expanded_List_Substitution(Implementation(CRM_Server_Login_i),STRING_WRITE)==(skip);
  Seen_List_Precondition(Implementation(CRM_Server_Login_i),INT_WRITE)==(vv: NAT);
  Seen_Expanded_List_Substitution(Implementation(CRM_Server_Login_i),INT_WRITE)==(skip);
  Seen_List_Precondition(Implementation(CRM_Server_Login_i),INTERVAL_READ)==(nn: NAT & mm: NAT & mm<=nn);
  Seen_Expanded_List_Substitution(Implementation(CRM_Server_Login_i),INTERVAL_READ)==(@(bb$0).(bb$0: mm..nn ==> bb:=bb$0));
  Seen_List_Operations(Implementation(CRM_Server_Login_i),Machine(BASIC_IO))==(INTERVAL_READ,INT_WRITE,STRING_WRITE);
  Seen_Expanded_List_Invariant(Implementation(CRM_Server_Login_i),Machine(BASIC_IO))==(btrue)
END
&
THEORY ListIncludedOperationsX IS
  List_Included_Operations(Implementation(CRM_Server_Login_i),Machine(BASIC_IO))==(INTERVAL_READ,INT_WRITE,STRING_WRITE)
END
&
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(CRM_Server_Login_i)) == (? | ? | ? | ? | access_grant | ? | seen(Machine(BASIC_IO)) | ? | CRM_Server_Login_i);
