...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(Development_Server_Login_i),connection_refuse)==(btrue | Same_Account_Connection = 0 ==> Account_Connection_Result:=Success [] not(Same_Account_Connection = 0) ==> Account_Connection_Result:=Failure);
  Expanded_List_Substitution(Implementation(Development_Server_Login_i),access_grant)==(Input_String_Length: NAT & Buffer_Length: NAT & Input_String_Length<Buffer_Length | Input_ID = OEM_TeamLeader_ID & Input_Password = OEM_TeamLeader_Password & Input_Agent = Developer_PC & Login_Try<=5 & Input_String_Length<Buffer_Length ==> (Agent:=Access_Success;Permission:=Upload_Fix_Transfer_Download_Approve;(0: INT | Login_Try:=0);("Login Success!\n": STRING | skip);("Login Entity: OEM_TeamLeader\n": STRING | skip);("============================\n": STRING | skip)) [] not(Input_ID = OEM_TeamLeader_ID & Input_Password = OEM_TeamLeader_Password & Input_Agent = Developer_PC & Login_Try<=5 & Input_String_Length<Buffer_Length) ==> (Input_ID = OEM_TeamMember_ID & Input_Password = OEM_TeamMember_Password & Input_Agent = Developer_PC & Login_Try<=5 & Input_String_Length<Buffer_Length ==> (Agent:=Access_Success;Permission:=Upload_Fix_Transfer;(0: INT | Login_Try:=0);("Login Success!\n": STRING | skip);("Login Entity: OEM_TeamMember\n": STRING | skip);("============================\n": STRING | skip)) [] not(Input_ID = OEM_TeamMember_ID & Input_Password = OEM_TeamMember_Password & Input_Agent = Developer_PC & Login_Try<=5 & Input_String_Length<Buffer_Length) ==> (Agent:=Access_Fail;Permission:=None;(Login_Try+1: INT & Login_Try: INT & 1: INT | Login_Try:=Login_Try+1);("Login Fail!\n": STRING | skip);("============================\n": STRING | skip))));
  List_Substitution(Implementation(Development_Server_Login_i),access_grant)==(IF Input_ID = OEM_TeamLeader_ID & Input_Password = OEM_TeamLeader_Password & Input_Agent = Developer_PC & Login_Try<=5 & Input_String_Length<Buffer_Length THEN Agent:=Access_Success;Permission:=Upload_Fix_Transfer_Download_Approve;Login_Try:=0;printf("Login Success!\n");printf("Login Entity: OEM_TeamLeader\n");printf("============================\n") ELSE IF Input_ID = OEM_TeamMember_ID & Input_Password = OEM_TeamMember_Password & Input_Agent = Developer_PC & Login_Try<=5 & Input_String_Length<Buffer_Length THEN Agent:=Access_Success;Permission:=Upload_Fix_Transfer;Login_Try:=0;printf("Login Success!\n");printf("Login Entity: OEM_TeamMember\n");printf("============================\n") ELSE Agent:=Access_Fail;Permission:=None;Login_Try:=Login_Try+1;printf("Login Fail!\n");printf("============================\n") END END);
  List_Substitution(Implementation(Development_Server_Login_i),connection_refuse)==(IF Same_Account_Connection = 0 THEN Account_Connection_Result:=Success ELSE Account_Connection_Result:=Failure END)
END
&
...
THEORY ListSetsX IS
  ...
  Set_Definition(Implementation(Development_Server_Login_i),Operation)==({None,All,Upload,Fix,Transfer,Download,Approve,Backup,Recovery,Install,Test,ECU_Data_Manage,Encrypt,Configuration_Manage,Upload_Fix_Transfer,Upload_Fix_Transfer_Download_Approve,Download_ECU_Data_Manage,Encrypt_Configuration_Manage});
...
THEORY ListSeenInfoX IS
  Seen_Internal_List_Operations(Implementation(Development_Server_Login_i),Machine(BASIC_IO))==(printf,scanf);
  ...
  Seen_List_Precondition(Implementation(Development_Server_Login_i),scanf)==(ss: STRING);
  Seen_Expanded_List_Substitution(Implementation(Development_Server_Login_i),scanf)==(skip);
  Seen_List_Precondition(Implementation(Development_Server_Login_i),printf)==(ss: STRING);
  Seen_Expanded_List_Substitution(Implementation(Development_Server_Login_i),printf)==(skip);
  Seen_List_Operations(Implementation(Development_Server_Login_i),Machine(BASIC_IO))==(printf,scanf);
  Seen_Expanded_List_Invariant(Implementation(Development_Server_Login_i),Machine(BASIC_IO))==(btrue)
END
&
THEORY ListIncludedOperationsX IS
  List_Included_Operations(Implementation(Development_Server_Login_i),Machine(BASIC_IO))==(printf,scanf)
END
&
