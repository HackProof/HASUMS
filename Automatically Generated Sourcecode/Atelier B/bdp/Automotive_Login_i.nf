...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(Automotive_Login_i),access_grant)==(Input_String_Length: NAT & Buffer_Length: NAT & Input_String_Length<Buffer_Length | Input_ID = Update_Server_ID or Input_ID = Engineer_ID & Input_Agent = Update_Server or Input_Agent = Engineer_PC & Login_Try<=5 ==> (Agent:=Access_Success;Permission:=Download;(0: INT | Login_Try:=0);("Login Success!\n": STRING | skip);("Login Entity: Update_Server\n": STRING | skip);("============================\n": STRING | skip)) [] not(Input_ID = Update_Server_ID or Input_ID = Engineer_ID & Input_Agent = Update_Server or Input_Agent = Engineer_PC & Login_Try<=5) ==> (Agent:=Access_Fail;Permission:=None;(Login_Try+1: INT & Login_Try: INT & 1: INT | Login_Try:=Login_Try+1);("Login Fail!\n": STRING | skip);("============================\n": STRING | skip)));
  List_Substitution(Implementation(Automotive_Login_i),access_grant)==(IF Input_ID = Update_Server_ID or Input_ID = Engineer_ID & Input_Agent = Update_Server or Input_Agent = Engineer_PC & Login_Try<=5 THEN Agent:=Access_Success;Permission:=Download;Login_Try:=0;printf("Login Success!\n");printf("Login Entity: Update_Server\n");printf("============================\n") ELSE Agent:=Access_Fail;Permission:=None;Login_Try:=Login_Try+1;printf("Login Fail!\n");printf("============================\n") END);
...
THEORY ListSetsX IS
  ...
  Set_Definition(Implementation(Automotive_Login_i),Operation)==({None,All,Upload,Fix,Transfer,Download,Approve,Backup,Recovery,Install,Test,ECU_Data_Manage,Encrypt,Configuration_Manage,Upload_Fix_Transfer,Upload_Fix_Transfer_Download_Approve,Download_ECU_Data_Manage,Encrypt_Configuration_Manage});
...
THEORY ListSeenInfoX IS
  Seen_Internal_List_Operations(Implementation(Automotive_Login_i),Machine(BASIC_IO))==(printf,scanf);
  ...
  Seen_List_Precondition(Implementation(Automotive_Login_i),scanf)==(ss: STRING);
  Seen_Expanded_List_Substitution(Implementation(Automotive_Login_i),scanf)==(skip);
  Seen_List_Precondition(Implementation(Automotive_Login_i),printf)==(ss: STRING);
  Seen_Expanded_List_Substitution(Implementation(Automotive_Login_i),printf)==(skip);
  Seen_List_Operations(Implementation(Automotive_Login_i),Machine(BASIC_IO))==(printf,scanf);
  Seen_Expanded_List_Invariant(Implementation(Automotive_Login_i),Machine(BASIC_IO))==(btrue)
END
&
THEORY ListIncludedOperationsX IS
  List_Included_Operations(Implementation(Automotive_Login_i),Machine(BASIC_IO))==(printf,scanf)
END
&
