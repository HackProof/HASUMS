...
{
    if(((Storage__File_Type == Storage__Binary) &&
        (Storage__Storage_Capacity == File_Size)) &&
...
{
    if(((Storage__Data_Status == Storage__Sourcecode) &&
        (Storage__Data_Status == Storage__Patchfile)) &&
    ((Current_Item_Version) < (Latest_Item_Version)))
    {
        Storage__Subject_Entity = Storage__OEM_TeamLeader;
        Storage__Data_Operation = Storage__Transfer;
    }
    else
    {
        Storage__Data_Operation = Storage__None;
